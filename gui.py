...
parser.add_argument(
    ...
    dest="names",
    type=str,
    help="Name of animals in order [1,2,3,4]",
)
...
parser.add_argument(
    ...
    dest="species",
    type=str,
    help="define the species to annotate primate/mouse/else",
)
...
def mold_image(img):
    image, window, scale, padding, crop = resize_image(
        img[:, :, :], min_dim=2048, min_scale=2048, max_dim=2048, mode="square"
    )
    return image
...
class WindowHandler:
    ...
    def __init__(
        ...
        cv2.namedWindow(
            self.window_name, cv2.WINDOW_NORMAL
        ...
        self.local_slider = cv2.createTrackbar(
            "Local Slider",
            self.window_name,
            self.local_slider_lower_window,
            self.local_slider_higher_window,
            self.on_change_local,
        )
        self.global_slider = cv2.createTrackbar(
            "Global Slider",
            self.window_name,
            self.current_frame,
            self.overall_frames - 1,
            self.on_change_global,
        )

    def load_frames(self, start, end):
        ...
        results_list = Parallel(
            n_jobs=40, max_nbytes=None, backend="multiprocessing", verbose=40
    ...
    def load_data(self):
        return np.load(
            self.results_sink + "IDresults_" + self.filename + ".npy", allow_pickle=True
        ).item()
    ...
    def mask_to_opencv(self, frame, mask, color, animal_id=None, mask_id=None):
    ...
    def display_mask(self, mask_id, current_mask):
        ...
        if self.current_frame == self.current_frame_focus:
            if mask_id == self.current_mask_focus:
                ...
                self.mask_to_opencv(
                    frame, current_mask, self.mask_color_default_focus_frame
                )
        else:
            if is_labeled:
                animal_id = self.results[self.current_frame]["results"][mask_id]
    ...
    def draw_random_frame(self, window=50):
        draw = 0
        ...
        while True:
            ...
            print(draw)
            if (
                window < draw < len(self.masks) - window
                and draw not in self.results.keys()
            ):
                print("True")
                break
            print("False")
    ...
    def adjust_trackbar(self):
        ...
        if (
            not self.local_slider_lower_window
            < self.current_frame
            < self.local_slider_higher_window
        ):
            self.local_slider_lower_window = (
    ...
    def set_new_regular_focus(self, interval=500):
        self.current_mask += 1
        self.current_frame_focus = (
            self.current_frame_focus + self.regular_focus_interval
        )
        self.current_frame = self.current_frame_focus
    ...
    def set_focus(self, focus_frame):
        # self.current_mask += 1
    ...
    def display_frame(self):
        if self.zoom:
            ...
            y1, x1, y2, x2 = self.masks[self.current_frame]["rois"][
                self.current_mask_focus
            ]
            ...
            # TODO: make relative
            masked_img = img[
                int(center_y - 200) : int(center_y + 200),
            ...
            cv2.imshow(
                "output",
                cv2.cvtColor(rescaled_img.astype("float32"), cv2.COLOR_BGR2RGB),
            )
        else:
    ...
    def display_all_indicators(self):
        for idx, indicator in enumerate(self.name_indicators.keys()):
            cv2.putText(
                ...
                self.name_indicators[indicator] + " : " + str(indicator + 1),
                (10, 850 + 25 * idx),
                self.font,
                0.5,
                (255, 255, 255),
    ...
    def display_all_keys(self):
        ...
        cv2.putText(
            self.frames[self.current_frame],
            "m -- trigger manual mode on current frame",
            (dist_1, 850 + 50),
            self.font,
            0.5,
            (255, 255, 255),
        ...
        cv2.putText(
            self.frames[self.current_frame],
            "h -- difficult to annotate (hard to see from current frame)",
            (dist_1, 850 + 100),
            self.font,
            0.5,
            (255, 255, 255),
        ...
        cv2.putText(
            self.frames[self.current_frame],
            ". -- change mask to focus on forward",
            (dist_2, 850 + 0),
            self.font,
            0.5,
            (255, 255, 255),
        ...
        cv2.putText(
            self.frames[self.current_frame],
            ", -- change mask to focus on backward",
            (dist_2, 850 + 25),
            self.font,
            0.5,
            (255, 255, 255),
    ...
    def save_mask_result(self, result):
        if self.current_frame == self.current_frame_focus:
            try:
            ...
            try:
                ...
                results[self.current_mask_focus] = (
                    result + "_" + self.current_difficulty_flag
                )
            except KeyError:
                # first result indicates mask_id, second indicates animal id
                results = {
                    self.current_mask_focus: result + "_" + self.current_difficulty_flag
                }
                self.results[self.current_frame]["results"] = results
            # change to next random frame if all masks labeled
            if (
                self.current_mask_focus
                == len(self.masks[self.current_frame_focus]["rois"]) - 1
            ):
    ...
    def display_current_frame(self):
        ...
        try:
            ...
            for mask_id, mask in enumerate(current_masks):
                self.display_mask(mask_id, mask)
        except IndexError:
            pass
        try:
            self.display_frame()
    ...
    def check_keys(self):
        ...
        if frameclick == ord("p") and self.previous_frame_focus is not None:
            if self.previous_frame_focus < len(list(self.results.keys())):
                self.current_frame = list(self.results.keys())[
        ...
        if frameclick == ord("."):
            if (
                self.current_mask_focus
                < len(self.masks[self.current_frame]["rois"]) - 1
            ):
                self.current_mask_focus += 1
...
def main():
    ...
    for video_id, filename in enumerate(videos):
